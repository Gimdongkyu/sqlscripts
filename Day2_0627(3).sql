...
--select 조호할 컬럼 목록 from 테이블이름 [where 조건식]; 모든 컬럼은 * 로 대체

SELECT name FROM TBL_MEMBER;
SELECT name,JOIN_DATE  FROM TBL_MEMBER;
SELECT * FROM TBL_MEMBER;
SELECT * FROM TBL_MEMBER WHERE name = '최다현'; -- 조건식은 컬럼이름으로 조회할 관게논리식 
...
SELECT * FROM TBL_MEMBER WHERE name LIKE '최다%';		--조회 결과는?
SELECT * FROM TBL_MEMBER WHERE name LIKE '%다현%';		--조회 결과는?
...
INSERT INTO tbl_member VALUES (1,'김모모','momo@naver.com','2020-03-02');
...
VALUES (3,'최다현','dahy@naver.com','2020-03-04 16:37');
...
INSERT INTO tbl_member VALUES (4,'쯔위','aaa@gamil.com',sysdate);

SELECT*FROM tbl_member;


--처음 만든 테이블 구조 중 mno 걸럼을 정밀도 5로 축소 변경
